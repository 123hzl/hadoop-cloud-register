package com.hzl.hadoop.util.seal;
...
public class SealUtil {
	...
	public static BufferedImage buildPersonSeal(int imageSize, int lineSize, SealFont font, String addString)
		...
		if (font.getFontText().length() == 2) {
			if (addString != null && addString.trim().length() > 0) {
				...
				bi = drawThreeFont(bi, g2d, font, lineSize, imageSize, fixH,
						fixW, true);
			} else {
			...
			if (addString != null && addString.trim().length() > 0) {
				font.setFontText(font.getFontText() + addString);
				bi = drawFourFont(bi, font, lineSize, imageSize, fixH,
						fixW);
			} else {
				font.setFontText(font.getFontText());
				bi = drawThreeFont(bi, g2d, font, lineSize, imageSize, fixH, fixW,
						false);
			}
		} else {
	...
	private static void drawArcFont4Circle(Graphics2D g2d, int circleRadius, SealFont font, boolean isTop) {
		...
		//3.字体样式
		int fontStyle = font.getBold() ? Font.BOLD : Font.PLAIN;
